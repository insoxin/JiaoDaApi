...
api.apiPermission = function(req, res, next) {
  ...
  if (id) {
    ...
    //根据appid读取app权限信息
    services.app_permission_model.findOne({
      appid: id
    }, function(err, app_permission) {
      if (err) {
        console.log(err);
        res.dump('redisError');
        return;
...
api.currentScore = function(req, res) {
  ...
  libs.get({
    ...
    password: password,
    url: "http://127.0.0.1/bxqcjcxAction.do?totalrows=16&pageSize=300"
  }, function(e, r) {
    if (e) {
      ...
      //libs.rePost(
      //    {
      //        url: 'http://127.0.0.1/logout.do?totalrows=300&pageSize=300',
      //        form: {
      //            'loginType': "platformLogin"
      //        },
      //        j: r.j
