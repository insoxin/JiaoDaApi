...
updates.term = function(cb) {
  if (datas.status.versionStatus && datas.status.termStatus) {
    ...
    libs.get(o, function(err, data) {
      ...
      dbs.updateVersion({
        fieldName: "term",
        field: "termLatestVersion",
        version: (datas.version.termLatestVersion + 1)
      }, function(e, r) {
        if (e) {
          cb(e);
          return;
    ...
    for (var i = 1; i < 25; i++) {
      for (var j = 1; j < 8; j++) {
        ...
        dates.push({
          weekId: i,
          week: j,
          start: datas.firstDay['2016-2017-2-1'] + (i - 1) * 7 * 24 * 60 * 60 + (k * 24 * 60 * 60),
          end: datas.firstDay['2016-2017-2-1'] + (i - 1) * 7 * 24 * 60 * 60 + (k * 24 * 60 * 60) + (24 * 60 * 60)
        });
      }
    }
...
updates.type = function(cb) {
  if (datas.status.versionStatus && datas.status.accountStatus) {
    ...
    libs.get(o, function(err, data) {
      ...
      dbs.updateVersion({
        fieldName: "type",
        field: "typeLatestVersion",
        version: (datas.version.typeLatestVersion + 1)
      }, function(e, r) {
        if (e) {
          cb(e);
          return;
...
updates.college = function(cb) {
  if (datas.status.versionStatus && datas.status.accountStatus) {
    ...
    libs.get(o, function(err, data) {
      ...
      dbs.updateVersion({
        fieldName: "college",
        field: "collegeLatestVersion",
        version: (datas.version.collegeLatestVersion + 1)
      }, function(e, r) {
        if (e) {
          cb(e);
          return;
    ...
    libs.rePost({
      ...
      if (list.length > 0) {
        conn.query({
          ...
          var listSql = [];
          for (var i = 0; i < list.length; i++) {
            listSql[i] = "('" + list[i].examName + "','" + list[i].name + "','" + list[i].start + "'," + list[i].end + "," + o.studentId + ",'" + datas.currentTerm.termId + "'," + list[i].week + ",'" + list[i].campusId + "','" + list[i].building + "','" + list[i].classroom + "'," + (parseInt(roww[0].examVersion) + 1) + ",'" + list[i].seat + "')";
          }
          sql = "insert into scu_exam (`examName`,`name`,`start`,`end`,`studentId`,`termId`,`week`,`campusId`,`building`,`classroom`,`version`,`seat`) VALUES " + listSql.join(',');
          conn.query({
            sql: sql
          }, function(eeeee) {
            if (eeeee) {
